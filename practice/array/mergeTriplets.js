...
class Solution {
  ...
  mergeTriplets(triplets, target) {
    ...
    for (let i = 0; i < t.length; i++) {
      ...
      for (let j = i + 1; j < t.length; j++) {
        if (
          Math.max(t[i][l], t[j][l]) <= target[l] &&
          Math.max(t[i][m], t[j][m]) <= target[m] &&
          Math.max(t[i][r], t[j][r]) <= target[r]
        ) {
          t[i] = [
            Math.max(t[i][l], t[j][l]),
            Math.max(t[i][m], t[j][m]),
            Math.max(t[i][r], t[j][r]),
          ];
        }
      }
  ...
  optimisedMergeTriplets(triplets) {
    let set = new Set();
